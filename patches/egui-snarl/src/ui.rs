    ...
    V: SnarlViewer<T>,
{
    ...
    for in_pin in inputs {
        ...
        inputs_ui.scope_builder(builder, |pin_ui| {
            ...
            // egui 0.36 decides "drag" the moment the pointer stops being a click hit on the
            // pin, and a pin is a few points wide on a touch screen: without the travel check a
            // tap starts (and on release, drops) a wire.
            if r.drag_started_by(PointerButton::Primary)
                && pin_ui.input(|i| i.pointer.is_decidedly_dragging())
            {
                if modifiers.command {
