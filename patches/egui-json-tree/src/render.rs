...
fn render_job(ui: &mut Ui, job: LayoutJob) -> Response {
    ...
    // On a touch screen the drag belongs to the enclosing ScrollArea, which registers its drag
    // widget before the content and so loses egui's last-registered-wins hit-test tie to every
    // token here. Mirrors what egui does for label selection in label.rs:159-166.
    let sense = if ui.input(|input| input.has_touch_screen()) {
        Sense::click()
    } else {
        Sense::click_and_drag()
    };
    ui.add(Label::new(galley).sense(sense))
}
