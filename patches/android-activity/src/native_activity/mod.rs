...
impl AndroidAppInner {
    ...
    pub fn show_soft_input(&self, show_implicit: bool) {
        // winit calls this with SHOW_IMPLICIT on the DecorView; when an EditText owns the
        // IME connection that show is ignored ("view is not served") and only spams logcat.
        // Explicit show (show_implicit=false) still runs for apps without an IME bridge.
        if show_implicit {
            return;
        }
        ...
        let res = self
            ...
            .attach_current_thread(|env| -> jni::errors::Result<()> {
                ...
                im_manager.show_soft_input(env, view, 0)?;
                Ok(())
            });
        if let Err(err) = res {
    ...
    pub fn hide_soft_input(&self, hide_implicit_only: bool) {
        // winit/egui-winit call this with HIDE_IMPLICIT_ONLY on every IME interrupt /
        // allow_ime toggle. With a focused EditText IME bridge the follow-up DecorView
        // showSoftInput is ignored ("view is not served"), so the hide permanently kills
        // the keyboard. Explicit hide (flags=0) still runs for app-owned dismissals.
        if hide_implicit_only {
            return;
        }
        ...
        let res = self
            ...
            .attach_current_thread(|env| -> jni::errors::Result<()> {
                ...
                let _hidden = imm.hide_soft_input_from_window(env, token, 0)?;
                Ok(())
            });
