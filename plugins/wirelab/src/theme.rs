//! Galactic-glass theme: an AMOLED-black page, violet glass surfaces, two neon accents.
//!
//! The grammar is comfyui-android's. Hot pink is the primary *interaction* signal (selected /
//! pressed / active), aqua is the secondary (hover, links, live markers), and violet is not a
//! signal at all — it is the ambient light every surface is tinted by, which is why it can be
//! everywhere without competing with the other two.
//!
//! Pane edges are dim WHITE hairlines. A hued outline is what stops a surface reading as glass:
//! real glass has no colour at its edge, only light catching the bevel.
//!
//! Two things carry the look, and both are easy to undo by accident:
//!
//! 1. **Surfaces stay translucent.** Every floating fill here has alpha < 255, guarded by a test.
//! 2. **The page is lit.** Translucent fills over pure black read as flat dark plastic; [`ambience`]
//!    puts three wide, very low-alpha light pools under everything for them to composite against.
//!
//! There is no real backdrop blur: the guest tessellates in wasm and `plugin-abi` drops every paint
//! callback before it reaches the host, so a grab-pass can never run here. Layered translucent
//! fills, a neutral rim and the light pools stand alone — comfyui-android's own no-GL fallback.
//!
//! Glyphs stay ASCII plus mainstream emoji — no icon font is loaded into this Context, so anything
//! outside egui's default coverage renders as a tofu box.

use egui_ios_plugin_sdk::egui;

use egui::containers::scroll_area::ScrollBarVisibility;
use egui::{Color32, CornerRadius, Stroke};

/// Primary accent — hot pink. Reserved for what is active or chosen.
pub const PINK: Color32 = Color32::from_rgb(255, 61, 139);
/// A lifted pink for ink and rings, where the base pink reads dim on pure black.
pub const PINK_BRIGHT: Color32 = Color32::from_rgb(255, 110, 168);
/// Secondary accent — aqua. Hover feedback, links, and live/ready markers.
pub const AQUA: Color32 = Color32::from_rgb(43, 226, 214);
/// A lifted aqua for text and warnings.
pub const AQUA_BRIGHT: Color32 = Color32::from_rgb(120, 240, 232);
/// Third colour — violet. Ambient light, not an interaction state.
pub const VIOLET: Color32 = Color32::from_rgb(163, 140, 255);
/// Body ink — cool near-white.
pub const INK: Color32 = Color32::from_rgb(233, 233, 239);
/// Dimmed ink for secondary lines (ids, units, hints).
pub const INK_DIM: Color32 = Color32::from_rgb(150, 148, 162);
/// A pane edge: a dim white hairline, never a coloured one.
pub const RIM: Color32 = Color32::from_rgba_premultiplied(46, 46, 52, 46);
/// A brighter hairline for surfaces meant to sit closest to the eye.
pub const RIM_BRIGHT: Color32 = Color32::from_rgba_premultiplied(72, 72, 80, 72);
/// Backdrop of a graph/circuit canvas — a hair off black so the page reads as a distinct layer.
pub const CANVAS: Color32 = Color32::from_rgb(3, 3, 5);
/// The glass a hand-painted surface (flow node, board body, minimap) is filled with.
pub const SURFACE: Color32 = Color32::from_rgba_premultiplied(16, 16, 25, 190);

fn rgba(r: u8, g: u8, b: u8, a: u8) -> Color32 {
    Color32::from_rgba_unmultiplied(r, g, b, a)
}

/// Widget rest fill: violet-cast translucent glass, raised to compensate for alpha over black.
fn fill_rest() -> Color32 {
    rgba(31, 28, 47, 165)
}
fn fill_weak() -> Color32 {
    rgba(25, 23, 38, 150)
}
fn fill_hover() -> Color32 {
    rgba(43, 226, 214, 42)
}
fn fill_active() -> Color32 {
    rgba(255, 61, 139, 54)
}

/// Install the theme over the SDK's Mastertech default, which is fully opaque and desktop-dense.
pub fn apply(ctx: &egui::Context) {
    let mut v = egui::Visuals::dark();

    // Not fully opaque: [`ambience`] paints below the panels and a solid page would hide it.
    v.panel_fill = rgba(0, 0, 0, 232);
    // Menus / modals: violet glass lifted off the page. Higher alpha than comfyui-android's 120
    // because nothing blurs the backdrop here, and light text needs a settled surface to sit on.
    v.window_fill = rgba(19, 17, 30, 232);
    // Wells (striped rows, TextEdit/ScrollArea, code blocks) sit inside those surfaces, so they
    // carry alpha too — an opaque well is a matte rectangle in the middle of the glass.
    v.faint_bg_color = rgba(18, 16, 26, 190);
    v.extreme_bg_color = rgba(14, 12, 22, 200);
    v.code_bg_color = rgba(11, 10, 18, 205);
    v.window_stroke = Stroke::new(1.2, RIM_BRIGHT);
    v.window_corner_radius = CornerRadius::same(8);
    v.menu_corner_radius = CornerRadius::same(8);
    v.window_shadow =
        egui::epaint::Shadow { offset: [0, 2], blur: 12, spread: 2, color: rgba(0, 0, 0, 200) };
    v.popup_shadow =
        egui::epaint::Shadow { offset: [0, 2], blur: 10, spread: 1, color: rgba(0, 0, 0, 170) };

    v.override_text_color = Some(INK);
    v.hyperlink_color = AQUA;
    v.warn_fg_color = AQUA_BRIGHT;
    v.error_fg_color = PINK;
    // egui also draws progress bars with this fill.
    v.selection.bg_fill = rgba(255, 61, 139, 140);
    v.selection.stroke = Stroke::new(1.4, PINK_BRIGHT);

    widget_palette(&mut v.widgets);

    v.striped = true;
    v.collapsing_header_frame = true;
    v.indent_has_left_vline = true;
    ctx.set_visuals(v);

    ctx.all_styles_mut(|s| {
        // Touch sizes over Mastertech's 3x3 spacing / 4x1 padding / 18px rows.
        s.spacing.item_spacing = egui::vec2(6.0, 6.0);
        s.spacing.button_padding = egui::vec2(10.0, 7.0);
        s.spacing.interact_size.y = 34.0;
        let mut scroll = egui::style::ScrollStyle::solid();
        scroll.bar_width = 14.0;
        scroll.handle_min_length = 28.0;
        scroll.bar_inner_margin = 2.0;
        s.spacing.scroll = scroll;
    });
}

/// The interaction grammar shared by every framed widget: rest = dark glass, hover = aqua edge,
/// press/active/selected = pink. Re-applied inside menus and per-node styles, which start from a
/// copy of the ambient style.
pub fn widget_palette(w: &mut egui::style::Widgets) {
    let radius = CornerRadius::same(5);
    let text_bright = Color32::from_rgb(248, 250, 252);

    w.noninteractive.bg_fill = rgba(18, 16, 28, 132);
    w.noninteractive.weak_bg_fill = rgba(14, 12, 22, 120);
    w.noninteractive.bg_stroke = Stroke::new(1.0, RIM);
    w.noninteractive.fg_stroke = Stroke::new(1.0, INK);
    w.noninteractive.corner_radius = radius;

    w.inactive.bg_fill = fill_rest();
    w.inactive.weak_bg_fill = fill_weak();
    w.inactive.bg_stroke = Stroke::new(1.0, RIM_BRIGHT);
    w.inactive.fg_stroke = Stroke::new(1.0, INK);
    w.inactive.corner_radius = radius;

    w.hovered.bg_fill = fill_hover();
    w.hovered.weak_bg_fill = fill_hover();
    w.hovered.bg_stroke = Stroke::new(1.5, rgba(43, 226, 214, 240));
    w.hovered.fg_stroke = Stroke::new(1.5, text_bright);
    w.hovered.corner_radius = radius;

    w.active.bg_fill = fill_active();
    w.active.weak_bg_fill = fill_active();
    w.active.bg_stroke = Stroke::new(1.7, rgba(255, 61, 139, 245));
    w.active.fg_stroke = Stroke::new(2.0, Color32::WHITE);
    w.active.corner_radius = radius;

    w.open.bg_fill = fill_rest();
    w.open.weak_bg_fill = fill_weak();
    w.open.bg_stroke = Stroke::new(1.3, rgba(43, 226, 214, 205));
    w.open.fg_stroke = Stroke::new(1.0, INK);
    w.open.corner_radius = radius;
}

/// Wide, very-low-alpha light pools across `rect`, so the glass over it has something to catch.
///
/// egui has no radial gradient; each pool is 16 concentric filled circles at `ring_alpha`, which
/// composites to a smooth falloff at a fixed 48 circles regardless of size. The alpha is per ring,
/// not the total — 16 rings at `a` reach `1-(1-a/255)^16` at the centre, so 5 already reads as a
/// painted background rather than as light. Useful values are 1-3.
///
/// The radii leave black between the pools on purpose: light everywhere is just a tinted page, and
/// the glass only reads as glass where a pane spans lit and unlit ground at once.
pub fn ambience(painter: &egui::Painter, rect: egui::Rect, ring_alpha: u8) {
    let d = rect.width().min(rect.height()).max(1.0);
    for (fx, fy, fr, color) in [
        (0.12, 0.14, 0.46, VIOLET),
        (0.94, 0.38, 0.38, AQUA),
        (0.46, 0.97, 0.42, PINK),
    ] {
        light_pool(painter, rect.lerp_inside(egui::vec2(fx, fy)), d * fr, color, ring_alpha);
    }
}

/// One pool of light: nested discs of a constant low alpha, largest first.
fn light_pool(
    painter: &egui::Painter,
    center: egui::Pos2,
    radius: f32,
    color: Color32,
    ring_alpha: u8,
) {
    const RINGS: usize = 16;
    let fill = rgba(color.r(), color.g(), color.b(), ring_alpha);
    for i in 0..RINGS {
        painter.circle_filled(center, radius * (1.0 - i as f32 / RINGS as f32), fill);
    }
}

/// Paint the lit black page every translucent surface composites against.
///
/// The host paints no backdrop behind a plugin viewport and the guest's root Ui is a bare
/// background layer, so without this the theme stands on whatever was already on screen.
pub fn paint_page(painter: &egui::Painter, rect: egui::Rect) {
    painter.rect_filled(rect, 0.0, Color32::BLACK);
    ambience(painter, rect, 1);
}

/// A hand-painted glass surface: translucent fill under a neutral hairline.
pub fn glass(
    painter: &egui::Painter,
    rect: egui::Rect,
    corner: impl Into<CornerRadius>,
    fill: Color32,
    rim: Color32,
) {
    let corner = corner.into();
    painter.rect_filled(rect, corner, fill);
    painter.rect_stroke(rect, corner, Stroke::new(1.0, rim), egui::StrokeKind::Inside);
}

/// Three expanded rounded rects under a surface, standing in for `Visuals::window_shadow` where the
/// surface is painted rather than framed.
pub fn glass_shadow(painter: &egui::Painter, rect: egui::Rect, corner: u8) {
    for (grow, alpha) in [(2_u8, 28_u8), (4, 20), (6, 12)] {
        painter.rect_filled(
            rect.expand(grow as f32),
            CornerRadius::same(corner.saturating_add(grow)),
            Color32::from_black_alpha(alpha),
        );
    }
}

/// Card frame for a content section: violet glass under a white rim.
pub fn card() -> egui::Frame {
    egui::Frame::new()
        .fill(rgba(20, 18, 34, 140))
        .stroke(Stroke::new(1.0, RIM))
        .corner_radius(CornerRadius::same(10))
        .inner_margin(12)
}

/// Frame for a bar pinned to a viewport edge: a touch more opaque than a card, no rounding.
pub fn bar() -> egui::Frame {
    egui::Frame::new().fill(rgba(14, 12, 24, 190)).inner_margin(egui::Margin::symmetric(8, 6))
}

/// Selectable button that keeps its frame when unselected — egui drops it, which reads as a plain
/// label rather than a control.
pub fn selectable<'a>(selected: bool, atoms: impl egui::IntoAtoms<'a>) -> egui::Button<'a> {
    egui::Button::selectable(selected, atoms).frame_when_inactive(true)
}

/// [`egui::Ui::selectable_label`] with a persistent frame plus a neon pink rim when selected —
/// egui's `interact_selectable` leaves `bg_stroke` off for the selected state, so the pink edge a
/// pressed button would show is painted here on top of the selection fill.
pub fn selectable_label<'a>(
    ui: &mut egui::Ui,
    selected: bool,
    text: impl egui::IntoAtoms<'a>,
) -> egui::Response {
    let resp = ui.add(selectable(selected, text));
    if selected {
        ui.painter().rect_stroke(
            resp.rect,
            CornerRadius::same(5),
            Stroke::new(1.6, PINK),
            egui::StrokeKind::Inside,
        );
    }
    resp
}

/// [`egui::Ui::selectable_value`] with a persistent frame.
pub fn selectable_value<'a, Value: PartialEq>(
    ui: &mut egui::Ui,
    current_value: &mut Value,
    selected_value: Value,
    text: impl egui::IntoAtoms<'a>,
) -> egui::Response {
    let mut response = selectable_label(ui, *current_value == selected_value, text);
    if response.clicked() && *current_value != selected_value {
        *current_value = selected_value;
        response.mark_changed();
    }
    response
}

/// Tap height for a menu row.
pub const MENU_ROW_H: f32 = 40.0;
/// Tallest a menu body grows before it scrolls instead of running off the screen.
pub const MENU_MAX_H: f32 = 320.0;

/// Give a menu's rows a framed, touch-sized look: egui's `menu_style` strips the rest-state fill
/// and every accent rim and squashes `button_padding` to 2x0, so an entry otherwise reads as bare
/// text on an 18px line. Call once at the top of a popup body; children inherit it.
pub fn menu_row_style(ui: &mut egui::Ui) {
    let s = ui.style_mut();
    s.spacing.button_padding = egui::vec2(10.0, 8.0);
    s.spacing.interact_size.y = MENU_ROW_H;
    s.spacing.item_spacing.y = 4.0;
    widget_palette(&mut s.visuals.widgets);
}

/// Takes `width` plus one gap off the trailing edge of a row; returns the row's right edge for
/// [`end_trailing`]. A `right_to_left` child never wraps and inherits the parent clip rect, so an
/// unbudgeted trailing widget runs leftward under the leading content.
pub fn reserve_trailing(ui: &mut egui::Ui, width: f32) -> f32 {
    let row = ui.max_rect();
    let reserved = if width > 0.0 { width + ui.spacing().item_spacing.x } else { 0.0 };
    ui.set_max_width((row.width() - reserved).max(0.0));
    row.max.x
}

/// Restores the row's right edge after [`reserve_trailing`]; `set_max_width` measures from the
/// cursor, not from the row start.
pub fn end_trailing(ui: &mut egui::Ui, right_edge: f32) {
    let cursor = ui.available_rect_before_wrap().min.x;
    ui.set_max_width((right_edge - cursor).max(0.0));
}

/// Menu button whose popup closes only on a click outside it or an explicit `ui.close()`, and
/// whose rows get [`menu_row_style`]. egui's `CloseOnClick` default closes a menu on any click
/// inside it — padding, scrollbar, a submenu row, or a drag-scroll short enough to read as a tap.
pub fn menu_button<'a, R>(
    ui: &mut egui::Ui,
    atoms: impl egui::IntoAtoms<'a>,
    content: impl FnOnce(&mut egui::Ui) -> R,
) -> Option<R> {
    use egui::containers::menu::{MenuButton, MenuConfig};
    let (_, inner) = MenuButton::new(atoms)
        .config(MenuConfig::new().close_behavior(egui::PopupCloseBehavior::CloseOnClickOutside))
        .ui(ui, |ui| {
            menu_row_style(ui);
            // A popup is an Area: `constrain` moves it but cannot shrink it, so an uncapped body
            // is clipped off the bottom of the screen rather than scrolled.
            scroll_vertical().max_height(MENU_MAX_H).show(ui, content).inner
        });
    inner.map(|r| r.inner)
}

/// Vertical scroll area; scrollbar only when content overflows.
pub fn scroll_vertical() -> egui::ScrollArea {
    egui::ScrollArea::vertical().scroll_bar_visibility(ScrollBarVisibility::VisibleWhenNeeded)
}

#[cfg(test)]
mod tests {
    use super::*;

    /// An opaque fill anywhere in a floating surface punches a matte hole through the glass: the
    /// light pools then show only in the gaps between widgets, which is indistinguishable from
    /// having no theme at all. Anything that floats has to stay translucent.
    #[test]
    fn floating_surfaces_stay_translucent() {
        let ctx = egui::Context::default();
        apply(&ctx);
        let v = ctx.style_of(egui::Theme::Dark).visuals.clone();
        for (what, c) in [
            ("panel_fill", v.panel_fill),
            ("window_fill", v.window_fill),
            ("widget rest", v.widgets.inactive.bg_fill),
            ("widget rest (weak)", v.widgets.inactive.weak_bg_fill),
            ("combo/menu open", v.widgets.open.bg_fill),
            ("noninteractive frame", v.widgets.noninteractive.bg_fill),
            ("hover", v.widgets.hovered.bg_fill),
            ("active", v.widgets.active.bg_fill),
            ("selection", v.selection.bg_fill),
            ("striped row", v.faint_bg_color),
            ("text well", v.extreme_bg_color),
            ("code block", v.code_bg_color),
            ("card", card().fill),
            ("bar", bar().fill),
            ("painted surface", SURFACE),
        ] {
            assert!(c.a() < 255, "{what} is opaque ({c:?}) — it punches a hole through the glass");
            assert!(c.a() > 0, "{what} is fully transparent ({c:?}) — the surface would vanish");
        }
    }

    /// Violet is the glass. It carries no interaction meaning, so the check is that surfaces are
    /// actually cast in it rather than neutral grey — blue leading red leading green, at any alpha.
    #[test]
    fn surfaces_carry_the_violet_cast() {
        let ctx = egui::Context::default();
        apply(&ctx);
        let v = ctx.style_of(egui::Theme::Dark).visuals.clone();
        for (what, c) in [
            ("window_fill", v.window_fill),
            ("widget rest", v.widgets.inactive.bg_fill),
            ("noninteractive frame", v.widgets.noninteractive.bg_fill),
            ("text well", v.extreme_bg_color),
            ("card", card().fill),
            ("painted surface", SURFACE),
        ] {
            assert!(c.b() > c.r() && c.r() >= c.g(), "{what} ({c:?}) is not violet-cast");
        }
        // The two interaction accents keep their own hues — violet must not have eaten them.
        assert_eq!(v.error_fg_color, PINK);
        assert_eq!(v.hyperlink_color, AQUA);
    }

    /// A trailing widget in a phone-width row: reserving its width first is the only thing that
    /// keeps it out from under the leading buttons, which is how the checks chip used to get cut
    /// off on the right.
    #[test]
    fn a_reserved_trailing_widget_stays_clear_of_the_leading_row() {
        let ctx = egui::Context::default();
        apply(&ctx);
        let chip_w = 80.0;
        let lead = std::cell::Cell::new(egui::Rect::NOTHING);
        let chip = std::cell::Cell::new(egui::Rect::NOTHING);
        let row = std::cell::Cell::new(egui::Rect::NOTHING);
        let input = egui::RawInput {
            screen_rect: Some(egui::Rect::from_min_size(
                egui::Pos2::ZERO,
                egui::vec2(360.0, 640.0),
            )),
            ..Default::default()
        };
        let _ = ctx.run_ui(input, |ui| {
            ui.horizontal(|ui| {
                row.set(ui.max_rect());
                let edge = reserve_trailing(ui, chip_w);
                ui.scope(|ui| {
                    ui.style_mut().wrap_mode = Some(egui::TextWrapMode::Truncate);
                    lead.set(ui.button("🗺 Canvas").rect);
                    lead.set(lead.get() | ui.button("📁 a project with a very long name").rect);
                });
                end_trailing(ui, edge);
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    chip.set(ui.add_sized([chip_w, 30.0], egui::Button::new("✔ checks")).rect);
                });
            });
        });
        assert!(
            chip.get().max.x <= row.get().max.x + 0.5,
            "chip {:?} overflows the row {:?}",
            chip.get(),
            row.get()
        );
        assert!(
            chip.get().min.x >= lead.get().max.x,
            "chip {:?} runs under the leading buttons {:?}",
            chip.get(),
            lead.get()
        );
    }

    /// Mastertech's 18px rows and 4x1 button padding are desktop density; every one of them has to
    /// be overridden or the plugin keeps unhittable targets.
    #[test]
    fn touch_targets_survive_the_mastertech_default() {
        let ctx = egui::Context::default();
        apply(&ctx);
        let s = ctx.style_of(egui::Theme::Dark).spacing.clone();
        assert!(s.interact_size.y >= 32.0, "rows are {}px tall", s.interact_size.y);
        assert!(s.button_padding.y >= 6.0, "button padding is {:?}", s.button_padding);
        assert!(s.item_spacing.x >= 6.0, "item spacing is {:?}", s.item_spacing);
    }
}
